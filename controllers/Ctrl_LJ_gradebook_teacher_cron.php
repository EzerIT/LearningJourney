...
class Ctrl_LJ_gradebook_teacher_cron extends MY_Controller {

    public function index() {
        $this->get_grading_system_cron();
	}
	
    public function get_grading_system_cron()
    {
    	$this->load->model('Mod_LJ_gradebook_teacher');
    	$this->load->model('Mod_users');
    	$this->load->helper('lj_Student');
    	$this->lang->load('lj_grading_system', $this->language);

        $this->Mod_LJ_gradebook_teacher->delete_all_cron();
        $users = $this->Mod_LJ_gradebook_teacher->get_usersCron();
        ...
        foreach($users as $u)
        {
            ...
            $pi->set_userid($u->id);
            $duration = $this->Mod_LJ_gradebook_teacher->get_duration($u);
            $graders = $this->Mod_LJ_gradebook_teacher->get_userResults_teacher($u);
            $start = $this->Mod_LJ_gradebook_teacher->get_start($u);
            $suspiciousData = $this->Mod_LJ_gradebook_teacher->get_suspiciousData($u);
            $sumData = $this->Mod_LJ_gradebook_teacher->get_sumAnswer($u);
            $sumRight = 0;
            $sumWrong = 0;
        ...
        foreach ($objectarray as $u)
        {
            ...
            $this->Mod_LJ_gradebook_teacher->add_toDB($class, $grade[2], round($grade[0], 2), $grade[1], $duration, $progress, $suspiciousData, $name, $userid);
        }
    ...
    public function set_grade($user, $start, $grade)
    {
        ...
        if ($grade>0)
        {
            $this->Mod_LJ_gradebook_teacher->set_grade($user, $start, $grade);
        }
    }
    
    public function compare_grade($user)
    {
        $max = $this->Mod_LJ_gradebook_teacher->get_maxTimestamp($user);
        $current = $this->Mod_LJ_gradebook_teacher->get_lastDate($user);
    ...
    {
        echo "Works!";
        $this->Mod_LJ_gradebook_teacher->add_toDB2();
    }
