...
class Mod_LJ_gradebook_teacher extends CI_Model {
    ...
    public function get_usersInClassForTeacher($myID)
    {
    ...
    public function get_userResults_teacher($u)
    {
    	$query = $this->db->query("SELECT q.grading, q.userid, q.id, SUM(rf.correct) cor, SUM(q.end-q.start)                                             As sumdur, SUM(1-correct) AS wrong, (end- start)/sum(correct) AS sec_per_cor
                                    ...
                                    WHERE q.userid=$u->id AND (grading=1 OR grading IS NULL)
    ...
    public function get_data_cron_teacher(){

        $query = $this->db->query("SELECT q.grading, q.userid, q.id, SUM(rf.correct)cor, SUM(q.end-q.start) As sumdur,                                   SUM(1-correct) AS wrong, (end-start)/sum(correct) AS sec_per_cor
                                    FROM {PRE}sta_quiz AS q
                                    JOIN {PRE}sta_question AS quest ON q.id = quest.quizid AND q.userid = quest.userid
                                    JOIN {PRE}sta_requestfeature AS rf ON quest.id = rf.questid AND quest.userid = rf.userid
                                    GROUP BY q.grading, q.userid, q.id AND (grading=1 OR grading IS NULL)");
        return $query->result(); 
     ...
     public function get_data_cron_student(){
    ...
    public function add_toDB2() {
        
        $trainingtime = '"' . "100:27:23" . '"';
         
        $this->db->query("UPDATE {PRE}sta_grading_system SET trainingtime = $trainingtime WHERE userid=724");
